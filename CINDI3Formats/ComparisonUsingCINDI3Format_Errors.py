...
def PlotScatSun(d, iPRef, sG, sP, iVers):
    a2DatRef = d[iPRef][sG]
    # Ref date
    ...
    sFname = '{}_{}_{}.png'.format(sG, sP, iVers)
    sFname = sFname.replace('/', '_')
    savefig(sFname, dpi=300., bbox_inches='tight')
    close()


def PlotScatSunTimeSeries(d, sG, sP, iVers):
    h = []
    f, axAll = subplots(10, 1, sharex=True, sharey=True)
    f.set_size_inches(15 / 2.58, 20. / 2.58)
    axAll = axAll.ravel()
    ...
    for iEI, iEC in enumerate(lE):
        ...
        for iPI, iPC in enumerate(lP):
            ...
            if any(a2Dat):
                ...
                if iEI == 0:
                    h += ax.plot(num2date(dt[bG]), a2Dat[sP][bG], 'o', markersize=masi-masi*0.5, markeredgecolor=color[iPI],
                                 markerfacecolor='none', label='Pan' + str(iPC), alpha=0.6)
                else:
                    ax.plot(num2date(dt[bG]), a2Dat[sP][bG], 'o', markersize=masi-masi*0.5, markeredgecolor=color[iPI],
                                 markerfacecolor='none', alpha=0.6)
                ax.set_title('VEA {}'.format(iEC))
                # ax.set_ylim([0, 10])
                ax.set_ylim([0, 0.001])
                ax.xaxis.set_major_locator(DayLocator(interval=1))
                ax.xaxis.set_major_formatter(DateFormatter('%dth'))

                # ax.locator_params(axis='y', nbins=5)
            # ax.locator_params(axis='x', nbins=5)

    f.suptitle('{}_{}_{}'.format(sG, sP, iVers), fontsize=12)
    ...
    #        borderaxespad=0.,
    #        bbox_transform=gcf().transFigure, fancybox=True, prop={'size': 8}, framealpha=0.0)
    sFname = '{}_{}_{}_timeseries.png'.format(sG, sP, iVers)
    sFname = sFname.replace('/', '_')
    savefig(sFname, dpi=300., bbox_inches='tight')
    close()


def PlotDirSun(d, iPRef, sG, sP, iVers):
    a2DatRef = d[iPRef][sG]
    # Ref date
    dtRef, dtBlickRef = DtConverstion(a2DatRef['DOY'], a2DatRef['UTC'])
...
def PlotDirSunTimeSeries(d, sG, sP, iVers):
...
def main():
    ...
    global sPthDat, lD, lP, lE, iVers, lPoR, lG, dHL, fMaxTimeDif, masi, color
    ...
    sPthDat = 'C:/Blick/data/CINDI2/BlickP_CINDI_2_files_Modified_filtered_v2/'
    ...
    for sG in lG:
        for sP in lProd:
            ...
            if doPlot:
                ...
                print sG, sP
                if iVers <= 2:
                    PlotScatSunTimeSeries(d, sG, sP, iVers)
                    # PlotScatSun(d, iPRef, sG, sP, iVers)
                elif iVers == 4:
                    # PlotDirSunTimeSeries(d, iPRef, sG, sP, iVers)
                    PlotDirSun(d, iPRef, sG, sP, iVers)
